 ...
 // TODO: replace the hard-coded AXI width (256-bit here) with a parameter
 
`timescale 1 ns / 1 ps

	module MAXI_ip_doublewidth #
	(
		...
		parameter integer C_M_AXI_BURST_LEN	= `DL2block/256,
		parameter integer C_M_AXI_BURST_LEN_LOG2 = `DL2block_Log2-8, // Required here for Vivado >= 2020.2
		// Thread ID Width
    ...
    // parameter integer C_M_AXI_BURST_LEN_LOG2 = `DL2block_Log2-8; // moved to interface for Vivado 2020.1
...
//icount;acceptingB,readyB,
    ...
    genvar j;
    for (j=0; j<`DL2block/`DL2subblocks/256; j=j+1) begin
        assign dinB[(j+1)*256-1-:256]=FIFOin[j];
    end
	    ...
	    if (reset_sig) begin                                                       
	      ...
	      //axi_wdata <= 'b0;        
	        ...
	        if (weB ) begin
	           ...
	           for (i=0; i<`DL2block/`DL2subblocks/256; i=i+1) begin
	               FIFOout[(doutBstrobe*(`DL2block/`DL2subblocks/256))|i]<=
	                   doutB[(i+1)*256-1-:256]; 
	        ...
	        if (rnext)
                FIFOin[FIFOin_i/*+rnext*/]<=M_AXI_RDATA;       
            FIFOin_i<=FIFOin_i+rnext;
            
            readyB<=0;
            if ((FIFOin_i==`DL2block/`DL2subblocks/256-1) && rnext) begin
                readyB<=1;
            end
            if (readyB) 
                dinBstrobe<=dinBstrobe+1;
